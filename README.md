...
An interactive app designed for Open OnDemand that launches an RStudio Server
within a batch job.

Adapted from [the OSC RStudio Server version](https://github.com/OSC/bc_osc_rstudio_server),
but we've replaced so much that it didn't really feel accurate to call it a "fork".
...
- [Environment Modules] 4.0.0+ or any other `module restore` and `module load <modules>` based
  CLI used to load appropriate environments within the batch job before
  launching the RStudio Server.
- A Singularity image similar to [nickjer/singularity-rstudio] designed to run `rserver`

The original app claims that [Singularity] 2.4.2+ is required, but I'm not sure what causes that restriction.
...
1. Fork it ( <https://github.com/UCL-ARC/ood-rstudio-app/fork> )
